...
class AuthoritativeMatchEventBridge(
    ...
    fun accept(envelope: ServerEnvelope): Boolean {
        if (envelope.origin != Origin.AUTHORITATIVE_SERVER) { view.recordRiskSignal(false); return false }
        if (expectedMatchId.isBlank() || envelope.matchId != expectedMatchId) { view.recordRiskSignal(false); return false }
        ...
        if (envelope.localTick != null && envelope.authoritativeTick != null) view.recordAuthoritativeTick(envelope.localTick, envelope.authoritativeTick)
        if (envelope.type != "MEMORABLE_CONFIRMED") return false
        if (envelope.eventId.isBlank() || envelope.sequenceId.isBlank()) return false
        if (envelope.similarity !in 0.95..1.0) return false
        if (envelope.setNumber < 1 || envelope.deterministicReplayRef.isBlank()) return false
        ...
        deliveredEventIds.add(envelope.eventId); deliveredSequenceIds.add(envelope.sequenceId)
        return try {
            val registered = view.registerMemorableReplay(
                MemorableReplay(envelope.eventId, envelope.sequenceId, envelope.setNumber, envelope.player, envelope.similarity, envelope.deterministicReplayRef)
            )
            if (!registered) throw IllegalArgumentException("duplicate replay")
            view.onMemorableConfirmed(envelope.player, envelope.total, envelope.localCount, envelope.opponentCount)
            true
        } catch (_: IllegalArgumentException) {
            deliveredEventIds.remove(envelope.eventId); deliveredSequenceIds.remove(envelope.sequenceId); false
        }
    }
}
